...
/**
 * Final scoreboard: a SINGLE winner (lowest cumulative total), ranked low to
 * high, with shared rank on ties. "Play again" reseats the same players into
 * a fresh room; "Back to home" returns to browse.
 */
  ...
  return (
    <Overlay>
      <div className="w-full max-w-lg rounded-2xl bg-surface p-6 shadow-table ring-1 ring-gold/30">
        ...
        <p className="mt-1 text-center text-sm text-muted">Lowest total wins — first to 100+ ends the game</p>
        <SuitDivider className="my-4" />

        <ul className="flex flex-col gap-2">
          {result.standings.map((s) => {
            const color = POS_COLOR[relativePosition(s.seat, youSeat, numPlayers)];
